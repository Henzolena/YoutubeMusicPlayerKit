...
public extension YoutubeMusicPlayerKit {
    ...
    /// - Important: This function only executes if the configuration's ``YoutubeMusicPlayerKit/FullscreenMode`` of the ``YoutubeMusicPlayerKit/Configuration`` is set to `.web`.
    ...
    /// - Important: This property only indicates the fullscreen state when the ``YoutubeMusicPlayerKit/FullscreenMode`` of the ``YoutubeMusicPlayerKit/Configuration`` is set to `.web`.
    /// **It does not reflect the fullscreen state when playing a video in fullscreen using the `.system` mode.**
    /// - SeeAlso: ``YoutubeMusicPlayerKit/fullscreenStatePublisher``
    ...
    /// - Important: The value of this publisher only indicates the fullscreen state when the ``YoutubeMusicPlayerKit/FullscreenMode`` of the ``YoutubeMusicPlayerKit/Configuration`` is set to `.web`.
    /// **It does not reflect the fullscreen state when playing a video in fullscreen using the `.system` mode.**
    /// - SeeAlso: ``YoutubeMusicPlayerKit/fullscreenStatePublisher``
    @available(iOS 16.0, macOS 13.0, visionOS 1.0, *)
