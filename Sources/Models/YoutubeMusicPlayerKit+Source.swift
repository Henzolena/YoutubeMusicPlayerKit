...
// MARK: - YoutubeMusicPlayerKit+Source
...
extension YoutubeMusicPlayerKit.Source: Identifiable {
...
public extension YoutubeMusicPlayerKit.Source {
...
extension YoutubeMusicPlayerKit.Source: ExpressibleByArrayLiteral {
    
    /// Creates a new instance of ``YoutubeMusicPlayerKit/Source``
...
extension YoutubeMusicPlayerKit.Source: ExpressibleByURL {
...
extension YoutubeMusicPlayerKit.Source: Codable {
        ...
        if let urlString = try? decoder.singleValueContainer().decode(String.self),
           let source = Self(urlString: urlString) {
            self = source
                  ...
                  !videoIDs.isEmpty {
            self = .videos(ids: videoIDs)
        } else if let container = try? decoder.container(keyedBy: YoutubeMusicPlayerKit.Parameters.CodingKeys.self) {
            if let listType = try? container.decode(YoutubeMusicPlayerKit.Parameters.ListType.self, forKey: .listType),
