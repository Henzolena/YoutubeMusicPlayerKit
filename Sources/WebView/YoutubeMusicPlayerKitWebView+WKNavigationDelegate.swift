...
// MARK: - YoutubeMusicPlayerKitWebView+WKNavigationDelegate

extension YoutubeMusicPlayerKitWebView: WKNavigationDelegate {
    ...
    func webView(
        ...
        if url.scheme == self.player?.configuration.htmlBuilder.youTubePlayerEventCallbackURLScheme,
           let playerEventName = url.host.flatMap(YoutubeMusicPlayerKit.Event.Name.init) {
            // Initialize player event
            let playerEvent = YoutubeMusicPlayerKit.Event(
                ...
                .first { $0.name == self.player?.configuration.htmlBuilder.youTubePlayerEventCallbackDataParameterName }
                    .flatMap(YoutubeMusicPlayerKit.Event.Data.init)
            )
...
// MARK: - YoutubeMusicPlayerKitWebView+validURLRegularExpressions
